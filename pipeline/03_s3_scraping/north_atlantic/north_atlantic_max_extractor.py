#!/usr/bin/env python3
"""
North Atlantic Seed Co Maximum Data Extractor v1.0
Proven Dutch Passion + Attitude methodology applied to North Atlantic's 2,727 strains
...
class NorthAtlanticMaxExtractor:
    def __init__(self):
        self.s3_client = boto3.client('s3')
        self.bucket_name = 'ci-strains-html-archive'
        self.base_url = 'https://northatlanticseed.com'
    ...
    def extract_media_assets(self, soup):
        ...
        for img in images:
            ...
            if src:
                ...
                # Categorize images
                if any(term in src.lower() for term in ['product', 'catalog', 'main']):
    ...
    def calculate_comprehensive_quality_score(self, strain_data):
        ...
        high_value_fields = {
            ...
            'parent_1', 'flowering_min', 'product_images'
        }
        
        # Standard fields
        standard_fields = {
            'strain_name', 'page_title', 'meta_description', 'jsonld_description',
            'spec_strain_type', 'total_image_count'
        }
    ...
    def maximum_extraction_pipeline(self, html_content, url):
        ...
        if not strain_data.get('strain_name') and not strain_data.get('jsonld_product_name'):
            # Try multiple selectors for North Atlantic
            selectors = ['h1', '.product-title', '.product-name', 'h2', 'title']
    ...
    def process_north_atlantic_strains(self, limit: Optional[int] = None):
        """Process all North Atlantic strains with maximum extraction"""
        
        logger.info("Starting North Atlantic Seed Co Maximum Extraction Pipeline")
        
        # Get North Atlantic URLs
        try:
        ...
        north_atlantic_urls = df[df['url'].str.contains('northatlanticseed', na=False)]
        
        if limit:
            north_atlantic_urls = north_atlantic_urls.head(limit)
        
        logger.info(f"Found {len(north_atlantic_urls)} North Atlantic URLs")
        ...
        for idx, row in north_atlantic_urls.iterrows():
            ...
            try:
                ...
                if processed % 100 == 0:
                    logger.info(f"Processed {processed}/{len(north_atlantic_urls)} strains")
        ...
        # Save main dataset
        output_file = 'north_atlantic_maximum_extraction.csv'
    ...
    def generate_extraction_report(self, df, output_file):
        ...
        report = f"""# North Atlantic Seed Co Maximum Extraction Report

**Date:** {datetime.now().strftime('%B %d, %Y')}  
**Processor:** North Atlantic Maximum Value Extractor v1.0  
**Logic designed by Amazon Q, verified by Shannon Goddard.**
...
**Processing completed with proven Dutch Passion + Attitude methodology.**  
**Logic designed by Amazon Q, verified by Shannon Goddard.**
"""
        
        with open('north_atlantic_extraction_report.md', 'w', encoding='utf-8') as f:
            f.write(report)
        
        logger.info("Comprehensive extraction report generated")

def main():
    extractor = NorthAtlanticMaxExtractor()
    df = extractor.process_north_atlantic_strains()
...
This extraction uses the proven Dutch Passion + Attitude 8-method pipeline:
...
Enhanced with North Atlantic-specific patterns for flowering time and yield data.
"""
    
    with open('methodology.md', 'w', encoding='utf-8') as f:
        f.write(methodology)
    
    print(f"\nNORTH ATLANTIC SEED CO EXTRACTION COMPLETE!")
