...
df = pd.read_csv('../input/remove/botanical_north_atlantic.csv', encoding='latin-1')
print(f"Loaded {len(df)} North Atlantic strains")

# North Atlantic doesn't have separate THC/CBD columns
df['thc_min'] = None
df['thc_max'] = None
df['thc_avg'] = None
df['cbd_min'] = None
...
# North Atlantic has combined height_raw (not split indoor/outdoor)
df[['height_indoor_cm_min','height_indoor_cm_max']] = df['height_raw'].apply(lambda x: pd.Series(parse_height(x)))
df['height_outdoor_cm_min'] = None
df['height_outdoor_cm_max'] = None

# North Atlantic has combined yield_raw
df[['yield_indoor_g_m2_min','yield_indoor_g_m2_max']] = df['yield_raw'].apply(lambda x: pd.Series(parse_yield(x)))
df['yield_outdoor_g_plant_min'] = None
df['yield_outdoor_g_plant_max'] = None

df.to_csv('../output/botanical_north_atlantic_normalized.csv', index=False, encoding='latin-1')
...
print("\nNorth Atlantic normalized successfully")
