...
df = pd.read_csv('../input/remove/botanical_gorilla.csv', encoding='latin-1')
print(f"Loaded {len(df)} Gorilla strains")

df[['thc_min','thc_max','thc_avg']] = df['thc_raw'].apply(lambda x: pd.Series(parse_thc_cbd(x)))
df['cbd_min'] = None
df['cbd_max'] = None
df['cbd_avg'] = None
df[['flowering_days_min','flowering_days_max','flowering_days_avg']] = df['flowering_raw'].apply(lambda x: pd.Series(parse_flowering(x)))
...
df.to_csv('../output/botanical_gorilla_normalized.csv', index=False, encoding='latin-1')
...
print("\nGorilla normalized successfully")
