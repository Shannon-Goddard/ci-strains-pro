...
df = pd.read_csv('../input/remove/botanical_neptune.csv', encoding='latin-1')
print(f"Loaded {len(df)} Neptune strains")

# Neptune has no botanical data - only lineage
df['thc_min'] = None
df['thc_max'] = None
df['thc_avg'] = None
df['cbd_min'] = None
...
df.to_csv('../output/botanical_neptune_normalized.csv', index=False, encoding='latin-1')
...
print("\nNeptune normalized successfully")
