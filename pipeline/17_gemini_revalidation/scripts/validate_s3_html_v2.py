...
INPUT_FILE = r"c:\Users\uthin\OneDrive\Desktop\ci-strains-pro\pipeline\17_gemini_revalidation\input\pipeline_16_no_avg.csv"
OUTPUT_FILE = r"c:\Users\uthin\OneDrive\Desktop\ci-strains-pro\pipeline\17_gemini_revalidation\output\s3_validation_v2_results.json"
BATCH_SIZE = 5
...
def validate_batch(batch_rows):
    """Validate batch using S3 HTML with standardization rules"""
    ...
    for _, row in batch_rows.iterrows():
        ...
        batch_data.append({
            ...
            "current_data": {
                ...
                "height_indoor_cm_min": row.get('height_indoor_cm_min') if pd.notna(row.get('height_indoor_cm_min')) else None,
                "height_indoor_cm_max": row.get('height_indoor_cm_max') if pd.notna(row.get('height_indoor_cm_max')) else None,
                "height_outdoor_cm_min": row.get('height_outdoor_cm_min') if pd.notna(row.get('height_outdoor_cm_min')) else None,
                "height_outdoor_cm_max": row.get('height_outdoor_cm_max') if pd.notna(row.get('height_outdoor_cm_max')) else None,
                "yield_indoor_g_m2_min": row.get('yield_indoor_g_m2_min') if pd.notna(row.get('yield_indoor_g_m2_min')) else None,
                "yield_indoor_g_m2_max": row.get('yield_indoor_g_m2_max') if pd.notna(row.get('yield_indoor_g_m2_max')) else None,
                "yield_outdoor_g_plant_min": row.get('yield_outdoor_g_plant_min') if pd.notna(row.get('yield_outdoor_g_plant_min')) else None,
                "yield_outdoor_g_plant_max": row.get('yield_outdoor_g_plant_max') if pd.notna(row.get('yield_outdoor_g_plant_max')) else None
            }
        })
...
1. **Flowering times MUST be in DAYS**: If HTML shows weeks, convert to days (1 week = 7 days)
   - Example: "10 weeks" → extract as 70 days
   - If HTML shows BOTH weeks AND days (e.g., "8-9 weeks" and "55-65 days"), PREFER the explicit days value
2. **Genetics percentages**: Must add up to 100% (Indica + Sativa = 100%)
3. **Genetics type**: Must be one of: "Indica Dominant", "Sativa Dominant", "Balanced Hybrid"
4. **Heights**: Convert all to centimeters (cm)
5. **Yields**: Indoor in g/m², outdoor in g/plant
...
[
  {{
    ...
    "fields": {{
      ...
      "flowering_min": {{}},
      "flowering_max": {{}},
      "height_indoor_cm_min": {{}},
      "height_indoor_cm_max": {{}},
      "height_outdoor_cm_min": {{}},
      "height_outdoor_cm_max": {{}},
      "yield_indoor_g_m2_min": {{}},
      "yield_indoor_g_m2_max": {{}},
      "yield_outdoor_g_plant_min": {{}},
      "yield_outdoor_g_plant_max": {{}}
    }}
  }}
]
...
print("🚀 Starting S3-based validation with standardization rules...\n")
