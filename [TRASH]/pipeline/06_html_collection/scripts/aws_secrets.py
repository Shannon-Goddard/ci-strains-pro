#!/usr/bin/env python3
"""
AWS Secrets Manager Integration for HTML Collection
...
class AWSSecretsManager:
    ...
    def get_html_collection_credentials(self) -> Dict[str, Optional[str]]:
        ...
        # Get Bright Data credentials from the correct secret (API method)
        bright_data_secret = self.get_secret('cannabis-brightdata-api')
        if bright_data_secret:
            # For Bright Data API method (same as pipeline 04)
            api_key = bright_data_secret.get('api_key')
            zone = bright_data_secret.get('zone')
            customer = bright_data_secret.get('customer')
            
            if api_key and zone:
                # API method: USERNAME = zone, PASSWORD = api_key, ENDPOINT = API URL
