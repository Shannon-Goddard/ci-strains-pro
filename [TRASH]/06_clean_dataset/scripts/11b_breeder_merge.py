...
Merges S3-extracted breeders with existing data and applies Step 10E rules.
...
print(f"  Before extraction: {df['breeder_name_raw'].notna().sum()} ({df['breeder_name_raw'].notna().sum()/len(df)*100:.1f}%)")
print(f"  After extraction: {df['breeder_name_merged'].notna().sum()} ({df['breeder_name_merged'].notna().sum()/len(df)*100:.1f}%)")

# Apply Step 10E standardization rules
...
# Save
df.to_csv('../../cleaning_csv/11b_breeder_merged.csv', index=False, encoding='utf-8')
df.head(100).to_csv('output/11b_breeder_merged_sample.csv', index=False, encoding='utf-8')
print(f"\nOutput: {len(df)} rows -> ../../cleaning_csv/11b_breeder_merged.csv")
